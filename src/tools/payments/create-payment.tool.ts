...
const invoicePaymentSchema = z.object({
  invoice_id: z.string().describe("Hashed invoice ID"),
...
const CreatePaymentTool = createTool(
  ...
  {
    client_id: z.string().describe("The client's hashed ID"),
    amount: z.number().describe("Total payment amount"),
    invoices: z.array(invoicePaymentSchema).optional().describe("Which invoices to apply payment to"),
    date: z.string().optional().describe("Payment date YYYY-MM-DD (default: today)"),
    type_id: z.string().optional().describe("Payment type (e.g. 1=bank transfer, 2=cash)"),
    transaction_reference: z.string().optional().describe("Reference number"),
